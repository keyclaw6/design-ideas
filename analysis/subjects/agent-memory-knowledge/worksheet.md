...
4. OpenViking MCP docs name **15 tools** on `:1933/mcp` — [openviking](../../tools/openviking.md). Local `pip install openviking` is **0.4.17.1**; `openviking-server doctor` passes Python / `x86_avx512` engine / AGFS 0.1.7 and fails Config/Embedding/VLM with no `ov.conf`. Studio is still a JS shell; remaining: one live `viking://` session after a provider config.
