...
4. OpenViking MCP docs name **15 tools** on `:1933/mcp` — [openviking](../../tools/openviking.md). Local **0.4.17.1** with `openviking[local-embed]` served on `127.0.0.1:1933` (`auth_mode=dev`). Live `viking://` write/read of `resources/probe/hello.md` (32 B); `ls viking://` → resources+user; MCP `initialize` 2025-11-25 / 15 tools. VLM unset so L0 abstracts stay “not ready.” Card stays `ready-with-gaps` (`thread-partial`). Remaining: a VLM-backed abstract, or `ov` CLI via `ovcli.conf`.
