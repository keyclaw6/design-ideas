...
- FreeToken split and prefill checkpoints — must-read long thread; need the repo README and one machine profile.
...
1. Record FreeToken’s GitHub URL and one `profile` output on a 4090-class box (or mark blocked if no GPU here).
