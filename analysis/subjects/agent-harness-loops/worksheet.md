...
1. popcorn CLI install/submit and the **live board** (sankalp1999 26th / 3916 µs vs blog 12th / 1805 µs) are on [codex](../../tools/codex.md). GPU Mode `GET /api/leaderboard/773` names Harbor `submission_id` **796445**. Blog re-fetched **200 / 158,069 B** (card `#c4`): **12th of 183**, **232×** from 419,000 → 1,805 µs. `submit_logs/` is a **local workspace folder** in that post, not a public GitHub tree. Remaining: do not keep hunting a public dump.
...
6. Ouroboros paper + `/benchmarks` + `razzant/ouroboros` are now on the card ([agent-harness-ops](../../techniques/agent-harness-ops.md)). Thread stays `failed` (2/0). Remaining: do not treat `Q00/ouroboros` as this paper.
...
8. Greg Mushen “Hermes” is an architecture layer name — do not collapse with `NousResearch/hermes-agent` MIT **241,533★** / homepage **90,409 B** v0.21.0 ([hermes](../../tools/hermes.md); [x-2093437790969385283#c3](../../items/x-2093437790969385283/card.md)).
