...
1. 61-id list + desktop URL detects + **mobile 390x844** (fixture 20 / blume 85 / frontal 138) are on [impeccable](../../tools/impeccable.md). nateherk.com and complete-shelf: stock npx **30 s** timeout (`[]`, exit 0); local patch **90 s** still `networkidle0` timeout (`[]`, exit 1). Same patched binary still detects example.com (3 findings, exit 2). Remaining: a `waitUntil: load` pass; do not treat 4.2.0 as the 4.1 card.
