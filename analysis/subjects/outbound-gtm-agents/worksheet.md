# Judgment worksheet: outbound / GTM agents (outbound-gtm-agents)

Owner aliases: outbound, GTM, cold email, sales OS. Ranking and citation work stays in [serp-ai-visibility](../serp-ai-visibility/worksheet.md).

## outbound-gtm-agents — short stack to try

Three disconnected jobs. Do not wire them as one SaaS.

1. **People search with an eval.** People Search Bench / PeopleSearchBench (119 multilingual queries, live-web evidence) ([github-LessieAI-people-search-bench](../../items/github-LessieAI-people-search-bench/card.md), [web-arxiv-2603-27476](../../items/web-arxiv-2603-27476/card.md)). treg people-search is the metered router (README “60 providers”; live hero “47”) ([web-treg-people-search](../../items/web-treg-people-search/card.md), [x-2094740953554932149](../../items/x-2094740953554932149/card.md)).
2. **A sequence with a pipeline KPI.** Gojiberry 13-agent Sales OS ([github-romangojiberryAI-gojiberryai-sales-os](../../items/github-romangojiberryAI-gojiberryai-sales-os/card.md)). The must-read field note is 97 prospects / 1 demo in 24h ([x-2094326291906310180](../../items/x-2094326291906310180/card.md)). Instantly + MapsData is the capacity-math path ([x-2094927852399624557](../../items/x-2094927852399624557/card.md)), not an agent team.
3. **Channel order before volume.** Vacca warm-before-email playbook is the other must-read ([x-2094162743985308047](../../items/x-2094162743985308047/card.md)). Schneider’s “tight list + short offer” thread is the counter to infrastructure theater ([x-2094893065202803014](../../items/x-2094893065202803014/card.md)).

Graphed is FDE-built marketing agents on a warehouse ([web-graphed](../../items/web-graphed/card.md)) — a services motion, not a weekend install.

## outbound-gtm-agents — axis scores

| item | people-search precision/recall | paid data vendors | agent-runnable vs slideware | channel coverage | eval harness |
|---|---|---|---|---|---|
| People Search Bench | designed for it (119 queries) | uses live web, not a vendor score | runnable Python | search only | high |
| treg people-search | unknown (router, not a model) | high (people-search page **60** providers; `people.email.find` **from $0.0089**; companies.search Apollo **$0.026**) | high (one token) | enrichment only | none in this bank |
| Gojiberry Sales OS | n/a (roles after a list exists) | mid (MCP + LinkedIn) | high (13 markdown agents) | email / LinkedIn / qualify | none; 97/1 is a case study |
| Instantly + MapsData 10k/day | n/a | high (inboxes + scrape) | mid (formula, you operate it) | email | none |
| Vacca channel-order | n/a | unknown | playbook, not code | LinkedIn then email | none |
| Schneider IQ thread | n/a | low (list + copy) | essay | email | none |
| LinkedIn lead-magnet recipe | n/a | low | recipe | LinkedIn | none |
| Nine Grok bots | n/a | unknown | capability copy | ads / SEO / GEO claimed | none |
| Graphed | n/a | high (warehouse + FDE) | services | ads / SEO / cold | unknown |
| MapsData pricing promo | n/a | paid scrape | dashboard | Maps leads | none |

## outbound-gtm-agents — claims that need a receipt

- 97 prospects → 1 demo in 24h — must-read; need the ICP definition and whether the demo was booked or held.
- treg $0.0089 per lead and “1B+ contacts” — now first-party on `/people-search` (**83,902 B**): 1B+ / 60 providers / `people.email.find` from $0.0089. Do not collapse with companies.search Apollo $0.026 ([treg](../../tools/treg.md)). People Search Bench leaderboard (README): Lessie 65.2 / Exa 55.0 / Claude Code 46.0 / Juicebox 45.8 — **treg is not on the table** ([people-search-bench](../../tools/people-search-bench.md)).
- 10k emails/day via 500 inboxes / 167 domains — capacity formula, not a delivered log.
- MapsData ~$0.33–$0.98 per 1k leads — first-party on `/pricing`: those two numbers are **Scale monthly $0.33** and **Pro monthly $0.98**. Starter is **$1.90/1k**. **500 free leads a month** (refills). 100k per search is Scale only ([mapsdata](../../tools/mapsdata.md)).
- Nine Grok bots as a “full marketing stack” — same thin bundle already flagged on the SERP worksheet. leftover12 landing is `x.ai/bot/dep-tU0gmIPgiqNsvS4N4` (**96,215 B**, visible **297** chars): one third-party Dmitry bot, not a 9-bot inventory ([grok-bot](../../tools/grok-bot.md); [x-2095231184531828762#c4](../../items/x-2095231184531828762/card.md)).

## outbound-gtm-agents — do not treat as load-bearing

- Grok marketing-bot pack — wrappers, no audit artifact.
- CrowdReply Grok setup — citation outreach; primary is serp-ai-visibility.
- Vacca tweet 3× / 1,000 meetings / 5–11% — still tweet-only. Frontal case studies that *are* first-party: Playbook 20 opps / 20 days / 300+ replies; Teikametrics $300K+ / 9 mo; Aircall 3,655 accounts / 6 mo ([clay](../../tools/clay.md)).

## outbound-gtm-agents — next capture work

1. People Search Bench submission path is a CSV PR (`docs/submission_guide.md`, ≤15 hits/query) — [people-search-bench](../../tools/people-search-bench.md). Remaining: actually run treg + one BYO-key provider and store the score table.
2. leftover21: root t.co loops to the same tweet (`#c3`). ICP template already on [gojiberryai](../../tools/gojiberryai.md). 97/1 still tweet + Slack still. Remaining: a filled ICP for that run — do not invent one.
3. Vacca `linked-page-unfetched` cleared — frontal.so + three case studies + GTM blog are on [clay](../../tools/clay.md). All **25** `/blog` hrefs are now fetched (card `#c5`); none carry 3× deals / 1,000 meetings / 5–11%. Thread stays `captured_partial`. Remaining: a property + date window for those tweet claims. Instantly 10k/day stays capacity math.
4. MapsData pricing page is first-party ([x-2095214420398121034#c3](../../items/x-2095214420398121034/card.md); [mapsdata](../../tools/mapsdata.md)). Do not treat tweet $0.33–$0.98 as one plan.
5. leftover22 unused `b2bfunnel.co` **71,788 B** is a DFY agency at **2–3K** emails/day/client — not Instantly **10k/day** ([x-2094927852399624557#c4](../../items/x-2094927852399624557/card.md)). Do not invent `b2bfunnel.md`.
6. leftover23 unused abs `arxiv.org/abs/2603.27476` **43,633 B** PeopleSearchBench (v3): **119** queries / four scenarios / κ=**0.84**. Does not name treg #1 ([people-search-bench](../../tools/people-search-bench.md); [x-2094740953554932149#c4](../../items/x-2094740953554932149/card.md)). leftover23 unused Cal.com Graphed Discovery Call **576,201 B** is a booking SPA with empty visible text — not the MCP warehouse page ([graphed-mcp](../../tools/graphed-mcp.md); [x-2094893065202803014#c4](../../items/x-2094893065202803014/card.md)).
7. leftover24 unused GitHub `superdesigndev/treg` **1,190★** ([treg](../../tools/treg.md); [x-2094740953554932149#c5](../../items/x-2094740953554932149/card.md)). leftover24 unused `mapsdata.ai` **55,354 B** restates **500** free / **$3.30** per 10k ([mapsdata](../../tools/mapsdata.md)). leftover24 unused Gojiberry Sales OS GitHub still **96★** — still no filled ICP ([gojiberryai](../../tools/gojiberryai.md); [x-2094326291906310180#c4](../../items/x-2094326291906310180/card.md)).
8. leftover25 unused `linkedin.com/in/leadgenwiz` **999 / 1,530 B** — do not hammer ([x-2095060844547592437#c5](../../items/x-2095060844547592437/card.md)).
9. leftover26 unused `gojiberry.ai` homepage already leftover21; unused `mcp.gojiberry.ai/mcp` stays keyed — do not hammer. leftover26 unused treg / iannuttall GitHub host strings are leftover24. Still no filled ICP.
10. leftover27 unused `lessie.ai` **1,469,320 B** homepage comparison: Lessie **65.2** / Exa **55** / Claude **46** / Juicebox **45.8** on **119** queries ([lessie](../../tools/lessie.md); [github-LessieAI-people-search-bench#c3](../../items/github-LessieAI-people-search-bench/card.md)). leftover23 abs already has PeopleSearchBench **119** / κ=**0.84** and does not name treg #1. Do not collapse homepage **65.2** with treg #1. Remaining: run treg + one BYO-key provider.
11. leftover29 unused `treg.to/llms.txt` **40,139 B**: **2,600+ / 60+** and later **2,800+ / 60**; **$1.00** free; five MCP tools — do not collapse with leftover README **2,896 / 60** or homepage hero **2,630 / 47** ([treg](../../tools/treg.md); [web-treg-people-search#c3](../../items/web-treg-people-search/card.md)). leftover29 unused `graphed.com` **292,950 B**; example **$2.4k** ([graphed-mcp](../../tools/graphed-mcp.md); [web-graphed#c3](../../items/web-graphed/card.md)). Authed `treg catalog` still missing.
12. leftover30 unused HTML `arxiv.org/html/2603.27476v3` **229,610 B** restates leftover23 **119** / κ=**0.84** and leftover27 Lessie **65.2 ± 1.5** — does not name treg ([people-search-bench](../../tools/people-search-bench.md); [web-arxiv-2603-27476#c3](../../items/web-arxiv-2603-27476/card.md)). leftover30 unused `treg.to/people-search` leftover23 + `superdesigndev/treg` leftover24 + keyed Graphed MCP ([treg](../../tools/treg.md); [graphed-mcp](../../tools/graphed-mcp.md)). Authed `treg catalog` still missing.
