# Thread — Custom PCB autorouter finishes keyboard board in one minute on two layers (x-2091891133286605067)

**Status:** captured_partial · **Author thread:** none · **Replies reported:** 27 · **Captured:** 1 · **Relevant:** 1 · **Unfetched:** 26 · **Truncated:** False
**Fetch log:** x-web-dom→ok (2026-09-04T17:54:20Z); fxtwitter-api→ok (2026-09-04T17:54:20Z)
**Raw payloads:** `raw/items/x-2091891133286605067/thread-raw/x-web-dom-20260904T175320Z.html`

**Author continuation (0 posts).**
**Quoted.**
> **@paulcjh** · 2091891339315089681
> Few more things need to be added in, main one is netclasses for different trace widths/clearances but that's pretty trivial

**Relevant replies (1 of 1 captured).**
> **@paulcjh** · 2091897482191995272 · depth 1 · link
> @paulcjh This looks sweet!
> 
> I made something along these lines a few weeks ago, as a kicad plugin. I found what was most useful was the ability to specify and place subsystems together in optimal configuration.
> 
> My system let you autoroute a few nets at a time thereafter, but I found that https://t.co/nIk29PGTLk

**Dropped as noise:** 0 replies (praise, emoji, bots, unrelated promo).
