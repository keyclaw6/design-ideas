# Thread — Kun Chen YOLO-agent ops: disposable Nix machine plus gated secrets (x-2082316720086405524)

**Status:** captured_partial · **Author thread:** none · **Replies reported:** 41 · **Captured:** 3 · **Relevant:** 3 · **Unfetched:** 38 · **Truncated:** False
**Fetch log:** x-web-dom→ok (2026-09-04T17:54:20Z); fxtwitter-api→ok (2026-09-04T17:54:20Z)
**Raw payloads:** `raw/items/x-2082316720086405524/thread-raw/x-web-dom-20260904T175301Z.html`

**Author continuation (0 posts).**
**Quoted.**
**Relevant replies (3 of 3 captured).**
> **@kunchenguid** · 2082396216403403069 · depth 1 · link
> @kunchenguid We've built something that gives you the best of both worlds. 

Would love to get your feedback! 

https://t.co/OiQrgdhtYY

> **@HashgraphOnline** · 2082468381673099703 · depth 1 · answered-question
> @kunchenguid +1 to all of this! Though I'm probably even more cavalier about secrets and access my agents have through my various logins. Where we are super careful is around "write" access to production. Otherwise, the productivity gain from letting agents just do stuff can't be beat.

> **@CalvinGrunewald** · 2082413443303285195 · depth 1 · correction
> @kunchenguid Have you tried using a vps instead of your laptop? That is what I have been doing the past few months and it’s made a huge difference. Not only in case something goes wrong but also so my laptop isn’t on 24/7. I am also working a lot more from my phone using termius and herdr.

**Dropped as noise:** 0 replies (praise, emoji, bots, unrelated promo).
