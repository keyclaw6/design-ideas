# Opus-in-Smith prompt-to-manufacturable mini jet engine demo

`x-2089802212000362939` · x · demo-video · en · [source](https://x.com/archedotco/status/2089802212000362939) · [raw](../../../raw/items/x-2089802212000362939/)
**Author:** Arche (@archedotco) · **Published:** — · **Captured:** 2026-09-04T06:52:37Z
**Disposition:** analyze · **Readiness:** ready-with-gaps · **Gaps:** —
**Subject:** [ai-cad-hardware](../../subjects/ai-cad-hardware/brief.md) · **Also:** — · **Roles:** example · **Platforms:** other

**Summary.** Archedotco demo of a manufacturable mini jet engine designed with Opus inside Smith CAD, tagged #prompttoengine. Positions prompt-to-CAD for real industrial geometry rather than decorative meshes, useful as an existence proof for equipment marketing assets.
**Question it answers.** Can Opus inside Smith produce manufacturable jet-engine CAD geometry from prompts?

**Claims.**
- `x-2089802212000362939#c1` (result, stated) The shown mini jet engine was designed with Opus in Smith and is described as manufacturable. — evidence: "A manufacturable mini jet engine, designed with Opus in Smith." [post]
**Numbers.** —
**Recipe.** —
**Techniques.** —
**Tools.** —
**Links.** —
**Related items.** [x-2089717063921332378](../x-2089717063921332378/card.md), [x-2090535643353153833](../x-2090535643353153833/card.md), [x-2087272209429766596](../x-2087272209429766596/card.md), [github-oso95-scroll-world](../github-oso95-scroll-world/card.md)
**Media.**
`raw/items/x-2089802212000362939/media/media_0.jpg` (video, carries_technique=true) — 14-second vertical clip showing a rendered mini jet engine CAD model produced via Opus in Smith at 588x960.
**Thread.** captured_partial · reported 6 · captured 3 · relevant 1 · author thread: none → [thread.md](thread.md)
**Judge hints.** must_read: False · compare with: —
