# TinyShots macOS screenshot polish app with agent-facing CLI

`web-tinyshots` · website · product · en · [source](https://tinyshots.app) · [raw](../../../raw/items/web-tinyshots/)
**Author:** TinyShots (@TinyShots) · **Published:** — · **Captured:** 2026-09-02T20:56:00Z
**Disposition:** analyze · **Readiness:** ready · **Gaps:** —
**Subject:** [landing-ui-motion](../../subjects/landing-ui-motion/brief.md) · **Also:** — · **Roles:** tool · **Platforms:** cli, other

**Summary.** macOS menu-bar app for polished marketing screenshots—backgrounds, annotations, Vision background removal, and compositions—with a full CLI for batch presets aimed at Claude Code and other agents.
**Question it answers.** What offline tool polishes product screenshots and exposes a CLI for agent pipelines?

**Claims.**
- `web-tinyshots#c1` (capability, stated) TinyShots provides on-device Vision background removal and a CLI for batch agent workflows. — evidence: "Full command-line interface for batch processing and agent workflows" [linked-page]
**Numbers.** early bird price: 39 USD (linked-page)
**Recipe.** —
**Techniques.** —
**Tools.** [tinyshots](../../tools/tinyshots.md)
**Links.** product (https://tinyshots.app)
**Related items.** [web-tinyshelf](../web-tinyshelf/card.md), [web-animos-editor](../web-animos-editor/card.md), [web-lottiefiles](../web-lottiefiles/card.md), [github-nateherkai-scroll-craft](../github-nateherkai-scroll-craft/card.md), [web-dicebear](../web-dicebear/card.md)
**Media.** —
**Judge hints.** must_read: ['page.md'] · compare with: [web-tinyshelf](../web-tinyshelf/card.md), [web-animos-editor](../web-animos-editor/card.md)
