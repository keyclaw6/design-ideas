# Gojiberry CEO launch — 13-agent outbound tree on hosted MCP for Grok Bot

`x-2094892848042725416` · x · thread · en · [source](https://x.com/pierreeliottlal/status/2094892848042725416) · [raw](../../../raw/items/x-2094892848042725416/)
**Author:** Pierre-Eliott Lallemant (@pierreeliottlal) · **Published:** 2026-09-01T20:59:10Z · **Captured:** 2026-09-02T18:10:58Z
**Disposition:** analyze · **Readiness:** ready-with-gaps · **Gaps:** thread-partial
**Subject:** [outbound-gtm-agents](../../subjects/outbound-gtm-agents/brief.md) · **Also:** [mcp-and-agent-browsers](../../subjects/mcp-and-agent-browsers/brief.md) · **Roles:** example, claim-source · **Platforms:** grok-bot, mcp

**Summary.** Gojiberry CEO open-sources a 13-role outbound sales OS for Grok Bot: signal hunting through meeting qualification coordinated via hosted MCP, with propose-don't-send defaults and a comment-BOT lead magnet on a 39.9s launch video.
**Question it answers.** How does the CEO pitch the 13-agent Gojiberry outbound handoff loop on Grok Bot?

**Claims.**
- `x-2094892848042725416#c1` (capability, stated) The post introduces a full outbound team of thirteen specialized agents from Head of Sales through Sales Manager. — evidence: "┃  
┣ 📁 Head of Sales" [post]
- `x-2094892848042725416#c2` (recipe, stated) Agents hand work along a loop from Signal Hunter through Meeting Qualifier powered by the Gojiberry MCP. — evidence: "All powered by the [@gojiberryai](https://x.com/gojiberryai) MCP." [post]
- `x-2094892848042725416#c3` (capability, stated) Outreach Operator queues LinkedIn campaigns with zero sent until the user approves. — evidence: "┃ ┗ 📁 0 sent until you say so" [post]
**Numbers.** launch video duration: 39.9 seconds (media)
**Recipe.** —
**Techniques.** [outbound-agent-pipeline](../../techniques/outbound-agent-pipeline.md)
**Tools.** [gojiberry-mcp](../../tools/gojiberry-mcp.md)
**Links.** repo (https://github.com/romangojiberryAI/gojiberryai-sales-os), product (https://gojiberry.ai/), https://mcp.gojiberry.ai/mcp
**Related items.** [github-romangojiberryAI-gojiberryai-sales-os](../github-romangojiberryAI-gojiberryai-sales-os/card.md), [x-2095081419202560010](../x-2095081419202560010/card.md)
**Media.**
`raw/items/x-2094892848042725416/media/thumb.jpg` (image, carries_technique=false) — Square video thumbnail with the gojiberryAI wordmark on a light gray background.
`raw/items/x-2094892848042725416/media/video.mp4` (video, carries_technique=true) — 39.9-second 1080x1080 launch video walking through the thirteen-agent outbound sales OS tree and MCP install pitch.
**Thread.** captured_partial · reported 197 · captured 1 · relevant 0 · author thread: none → [thread.md](thread.md)
**Judge hints.** must_read: False · compare with: [github-romangojiberryAI-gojiberryai-sales-os](../github-romangojiberryAI-gojiberryai-sales-os/card.md), [x-2095081419202560010](../x-2095081419202560010/card.md)
