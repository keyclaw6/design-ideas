# CozyClay timeline slow-mo handles for in-editor camera timing

`x-2091722166685610284` · x · repo · en · [source](https://x.com/Yun_HDY/status/2091722166685610284) · [raw](../../../raw/items/x-2091722166685610284/)
**Author:** Yun (@Yun_HDY) · **Published:** — · **Captured:** 2026-09-04T06:49:33Z
**Disposition:** analyze · **Readiness:** ready-with-gaps · **Gaps:** thread-partial
**Subject:** [blockout-to-video-flythrough](../../subjects/blockout-to-video-flythrough/brief.md) · **Also:** — · **Roles:** tool, technique · **Platforms:** browser

**Summary.** Yun_HDY shows CozyClay adding slow-motion as draggable timeline handles so punch timing lands slower in-browser, linking the open-source NomaDamas/CozyClay previs repo.
**Question it answers.** How can slow-motion camera timing live on the timeline instead of a separate app?

**Claims.**
- `x-2091722166685610284#c1` (capability, demonstrated) CozyClay exposes slow-motion as stretchable timeline handles that slow impact timing in the same editor. — evidence: "its a handle on the timeline now
stretch it and the punch actually lands slower" [post]
**Numbers.** —
**Recipe.** —
**Techniques.** —
**Tools.** —
**Links.** repo (https://github.com/NomaDamas/CozyClay)
**Related items.** [x-2091577179914338583](../x-2091577179914338583/card.md), [x-2093377271771865267](../x-2093377271771865267/card.md)
**Media.** —
**Thread.** captured_partial · reported 3 · captured 1 · relevant 0 · author thread: none → [thread.md](thread.md)
**Judge hints.** must_read: False · compare with: —
