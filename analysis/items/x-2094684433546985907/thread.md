# Thread — Sami directory backlink tiers: 15 free DR sites for founders (x-2094684433546985907)

**Status:** captured_partial · **Author thread:** none · **Replies reported:** 23 · **Captured:** 3 · **Relevant:** 1 · **Unfetched:** 20 · **Truncated:** False
**Fetch log:** x-web-dom→ok (2026-09-04T17:54:20Z); fxtwitter-api→ok (2026-09-04T17:54:20Z)
**Raw payloads:** `raw/items/x-2094684433546985907/thread-raw/x-web-dom-20260904T175333Z.html`

**Author continuation (0 posts).**
**Quoted.**
> **@samigrows** · 2094093678080119153
> The fastest backlink you can get today is a directory submission. 90+  
> 
> Crunchbase, G2, Clutch, Product Hunt, Capterra.  All DR 70 plus. All free. All indexable within days. 
> 
> Most founders spend months chasing guest posts.  
> 
> Ten directory submissions this afternoon beats six https://t.co/ts63Vwsvs3

**Relevant replies (1 of 3 captured).**
> **@samigrows** · 2094781115110424813 · depth 1 · answered-question
> @samigrows agree. the real win is a fresh domain getting indexed and looking legit in the first 90 days, not the DR bump. do you bother with guest posts at all for a brand new site, or just hammer directories?

**Dropped as noise:** 2 replies (praise, emoji, bots, unrelated promo).
