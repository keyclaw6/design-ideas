# FleetingBits: Claude-generated explorable codebase diagrams with inspectable data dots

`x-2088016749849682120` · x · demo-image · en · [source](https://x.com/fleetingbits/status/2088016749849682120) · [raw](../../../raw/items/x-2088016749849682120/)
**Author:** FleetingBits (@fleetingbits) · **Published:** — · **Captured:** 2026-09-04T06:49:28Z
**Disposition:** analyze · **Readiness:** ready-with-gaps · **Gaps:** —
**Subject:** [infographics-diagrams](../../subjects/infographics-diagrams/brief.md) · **Also:** [agent-harness-loops](../../subjects/agent-harness-loops/brief.md) · **Roles:** technique, example · **Platforms:** claude-code

**Summary.** FleetingBits describes having Claude render codebases as interactive visual diagrams so he can discuss architecture with the agent; moving dots represent data snippets he can inspect in the UI.
**Question it answers.** How can an agent turn a codebase into an explorable visual diagram for paired discussion?

**Claims.**
- `x-2088016749849682120#c1` (capability, demonstrated) Claude can render a codebase as a visual diagram with moving data snippets the author can inspect while discussing the code. — evidence: "the moving dots are data snippets that i can inspect" [post]
**Numbers.** —
**Recipe.** —
**Techniques.** —
**Tools.** —
**Links.** —
**Related items.** [github-cathrynlavery-diagram-design](../github-cathrynlavery-diagram-design/card.md), [x-2087329201451855933](../x-2087329201451855933/card.md), [github-antvis-infographic](../github-antvis-infographic/card.md), [web-flint-chart](../web-flint-chart/card.md)
**Media.**
`media/media_0.jpg` (image, carries_technique=true) — Isometric rivers-of-empire system map with labeled evolution-loop blocks, pan/zoom controls, and a documentation sidebar.
**Thread.** captured_partial · reported 196 · captured 3 · relevant 3 · author thread: none → [thread.md](thread.md)
**Judge hints.** must_read: [] · compare with: —
