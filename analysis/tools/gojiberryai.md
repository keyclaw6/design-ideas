# Gojiberryai

**Slug:** `gojiberryai` · **Kind:** product · **URL:** https://x.com/pierreeliottlal/status/2094326291906310180 · **Canonical item:** [x-2094326291906310180](../items/x-2094326291906310180/card.md)
**Subjects:** [mcp-and-agent-browsers](../subjects/mcp-and-agent-browsers/brief.md), [outbound-gtm-agents](../subjects/outbound-gtm-agents/brief.md)
**Referenced by (1):**
- [GojiberryAI MCP plus GrokBot: 97 prospects and 1 demo in 24h](../items/x-2094326291906310180/card.md) — example, claim-source — outbound-gtm-agents

<!-- NOTES:START -->
**2026-09-04 capture — 97/1 case still tweet + Slack still.** Homepage `https://gojiberry.ai/` HTTP 200. The 97 contacted / 33 accepted / 15+ replies / 1 demo in 24h numbers are only on the X post + Calendly-via-Zapier Slack still (`raw/…/photo.jpg` shows 2026-08-31 18:00 UTC, CIO field). Repo has `icp-context.template.md` but no filled ICP for that run. Do not treat 97/1 as a scored funnel.

**2026-09-04 capture — ICP template fields.** `skills/sales-os/icp-context.template.md` (1,023 bytes, HTTP 200). Blank fields: Product (company, website, one-sentence, who we sell to company/title, status quo, named proof); ICP (industries, size, geos, must-have / nice-to-have titles, disqualifiers, real vs fake buying triggers); Voice (sender, tone, words use/never, CTA); Send policy (propose default | autonomous, min intent score, LinkedIn | email, after-hours, approval first-touch / follow-up / both). README first-five-minutes: fill that file, then “find 25 people… show the list before anyone is contacted.” Default **propose, don't send**. 97/1 still has no filled copy of this file.

Neighbor (parked; no starborn.md): 2026-09-05 `https://www.starbornai.com/` GET 200 / **90,168 B**. H1 Add $15K–$50K MRR in 90 days using LinkedIn alone. First-party: write posts, run DMs, book 20–30 qualified sales calls. Marketing outcome copy, not a ledger. Receipt `analysis/_work/captures/leftover-2026-09-05.json`.

**2026-09-05 homepage re-read for the CEO launch card.** Same `https://gojiberry.ai/` **1,154,716 B**. ICP filtering + Email waterfall (15+ providers) + CRM/API/MCP named. **Grok** string absent. 13-agent tree stays tweet/repo.

**2026-09-05 leftover24.** Unused GitHub `romangojiberryAI/gojiberryai-sales-os` MIT **96★** on the 97/1 must-read. Still no filled ICP. Receipt `leftover24-2026-09-05.json`.

**2026-09-05 leftover21.** Root `t.co/s0za9kxTZC` loops to the same X status. Still no filled ICP for 97/1.

**2026-09-05 leftover25.** Unused neighbor `linkedin.com/in/leadgenwiz` **999 / 1,530 B** — do not hammer. Receipt `leftover25-2026-09-05.json`.

**2026-09-05 leftover26.** Unused `gojiberry.ai` homepage already leftover21; unused `mcp.gojiberry.ai/mcp` stays keyed — do not hammer. Still no filled ICP. Receipt `leftover26-2026-09-05.json`.
<!-- NOTES:END -->
