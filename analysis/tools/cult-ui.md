...
**2026-09-04 capture — Originkit MCP hello JSON.** `GET https://mcp.originkit.dev/` 200, **1,095** B: `name=originkit-mcp` `version=1.0.0`; tools **`list_components`, `get_component`, `search`, `fetch`**; **`components`: 461**; `authRequired: true`; REST ` /v1/registry` + `/v1/components/:slug`. `GET /v1/registry` 200, **176,179** B, `count` **461**, **461** unique `name`s (sample `interactive-grid`, `light-glass-button`, …). Homepage RSC still **468** unique `…-gallery` slugs — do not collapse 461 with 468. Receipts: `analysis/_work/captures/originkit-mcp-hello.json`, `originkit-registry-meta.json`.

**2026-09-05 leftover24.** Unused `originkit.dev` **562,318 B** title Originkit — Free Animated component library for modern websites. Visible empty (SSR shell). Receipt `leftover24-2026-09-05.json`.
<!-- NOTES:END -->
