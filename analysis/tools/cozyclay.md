...
**2026-09-04 capture — `npm run verify:capture` timed out.** `mcp/verify-live-capture.mjs` with `CHROME_PATH=/usr/bin/google-chrome` (headless=new). Suite reached a connected live editor, then `capture_frame` returned `isError` text “Live editor error: Live editor timed out running capture_frame.” Assertion failed (`actual: true`). That run used the stock **5,000 ms** live-hub timeout and no SwiftShader flags.

**2026-09-04 capture — live `capture_frame` with SwiftShader + 30 s timeout.** Same clone, local-only patches: `qa-chrome.mjs` gained `--use-gl=angle --use-angle=swiftshader --ignore-gpu-blocklist --enable-webgl --disable-gpu-sandbox`; `live-hub.mjs` `DEFAULT_COMMAND_TIMEOUT_MS` **5,000 → 30,000**. `npm run verify:capture` exit **0** twice. Happy path: **640×360** PNG, `nonBlackPixels` **230,400** (full frame), `blackFrame` false, state hash stable across the capture. Happy compressed sizes **116,977** then **117,016** bytes (do not collapse). `frame_shot` five views all `behindCameraPlane: false` with visiblePixelCount **6144 / 7168 / 6144 / 7168 / 7168**. Cube occluder: visiblePixelCount **0** then **7168** after move. Artifact PNG **93,730** B banked at `analysis/_work/captures/cozyclay-capture/artifact-640x360.png` (valid 640×360 RGBA; grey mannequin, back/three-quarter on a grid). Rejected edges still fire: blackFrame / noCamera / oversize. Receipt: `analysis/_work/captures/cozyclay-capture/report.json`. This is still software GL, not a laptop GPU.

**2026-09-05 product page.** `https://cozyclay.org/` GET 200 / **22,437 B**. First-party: free OSS browser previs; Three.js + R3F; no Blender/Unity; one command; block scene / pose / camera then hand shots to a video model. Tweet ~30 s reframe is not on this HTML. Receipt `analysis/_work/captures/leftover4-2026-09-05.json`.
<!-- NOTES:END -->
