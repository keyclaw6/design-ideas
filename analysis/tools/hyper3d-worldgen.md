...
- Marketing home https://hyper3d.ai is **Rodin** (image/text → 3D asset), not a WorldGen explainer. Re-fetch 2026-09-04 evening: `https://hyper3d.ai/worldgen` still **404**. `https://hyper3d.ai/workspace/worldgen` HTTP **200**, **11,026**-byte login SPA (`data-app-booting`, no title/h1, sets `hyper3dWorkspaceVisited` cookie; no export/gltf/usd/login strings in the shell). CAST arXiv:2502.12894 still 200. No public export.
...
**2026-09-05 leftover19.** Re-fetched `arxiv.org/html/2502.12894` **200 / 376,208 B**. Title CAST: Component-Aligned 3D Scene Reconstruction from an RGB Image. Tables above unchanged. WorldGen user export still login SPA.
- Still no public WorldGen export, EULA, or weights in this bank. Do not schedule production on the trailer.

**2026-09-05 leftover14 attach.** Bee one-shot leftover ([x-2092242135504552118](../items/x-2092242135504552118/card.md)) names no product. leftover13 t.co loops to the same tweet. Do not invent Meshy/Tripo/WorldGen as that tool — this WorldGen path is a login SPA with no public mesh.

Lumera project page https://haidilao0328.github.io/Lumera/ is a 1 KB HTML shell that loads `/Lumera/assets/project2NoDemo-B9J-SJip.js` (605,695 bytes). Bundle cites **arXiv:2607.20889** “Engine-Native Editable 3D World Reconstruction with Objects and Lighting” and a YouTube embed `x7s8649kAO4`. Abstract (fetched): Lumera-2K from **2,513 UE5 projects**; 3.73M components, 63M object instances, 102.6K parametric lights, 95.1K camera views; Lumera-Box / Lumera-Light parse boxes and light tuples. **No GitHub / weights URL in the bundle.** Lucida `https://lucida-r2s.github.io/` re-fetch 200, **25,548** B, title **“Lucida | Composable real-to-sim scene modeling”** (ByteDance Seed / PKU / ZJU). The only GitHub URL on the page is the author user `github.com/OrangeSodahub` — still no repo, weights, or arXiv link in the HTML. Interactive 3D preview copy: “Could not open the interactive scene.”
<!-- NOTES:END -->
