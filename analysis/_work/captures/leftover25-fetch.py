#!/usr/bin/env python3
"""leftover25: leftover unused directories / product hosts. No 429 hammer."""
...
OUT = Path("/workspace/analysis/_work/captures/leftover25-2026-09-05")
...
URLS = {
    ...
    "blume-codes": "https://blume.codes",
    "refero-http": "http://styles.refero.design",
    "linkedin-leadgenwiz": "https://www.linkedin.com/in/leadgenwiz",
    "aicss-home": "https://www.aicss.dev/",
}
...
def main() -> None:
    ...
    for slug, url in URLS.items():
        ...
        print(
            f"  -> {pages[slug].get('status')} {pages[slug].get('bytes')} {pages[slug].get('final')} {pages[slug].get('title')}",
            flush=True,
        )
        time.sleep(0.45)
    path = Path("/workspace/analysis/_work/captures/leftover25-2026-09-05.json")
    path.write_text(
        json.dumps({"fetched_at": "2026-09-05T12:00:00Z", "pages": pages}, indent=2),
        encoding="utf-8",
    )
    print("wrote", path)
