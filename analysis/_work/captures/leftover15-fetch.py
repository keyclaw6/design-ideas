#!/usr/bin/env python3
"""leftover15: Driscoll-linked first-party essays. No Brave/429/403 hammer."""
...
OUT = Path("/workspace/analysis/_work/captures/leftover15-2026-09-05")
...
def summarize(rec: dict, slug: str) -> dict:
    ...
    low = visible.lower()
    out["has"] = {
        "15000": "15,000" in visible or "15000" in visible,
        "95%": "95%" in visible or "95 %" in visible,
        "llms.txt": "llms.txt" in low,
        "snowflake": "snowflake" in low,
        "turbopuffer": "turbopuffer" in low,
        "da vinci": "da vinci" in low or "davinci" in low,
    }
    return out
...
URLS = {
    ...
    "cerebras": "https://www.cerebras.ai/blog/how-we-built-our-knowledge-base",
...
def main() -> None:
    ...
    for slug, url in URLS.items():
        ...
        print(
            f"  -> {pages[slug].get('status')} {pages[slug].get('bytes')} {pages[slug].get('title')} has={pages[slug].get('has')}",
            flush=True,
        )
        time.sleep(0.4)
    path = Path("/workspace/analysis/_work/captures/leftover15-2026-09-05.json")
    path.write_text(json.dumps({"fetched_at": "2026-09-05T07:40:00Z", "pages": pages}, indent=2), encoding="utf-8")
    print("wrote", path)
